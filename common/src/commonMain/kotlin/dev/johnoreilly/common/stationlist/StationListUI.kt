...
fun MapView(stationList: List<Station>) {
    ...
    val cameraState =
        rememberCameraState(firstPosition = CameraPosition(target = cameraPosition, zoom = 12.0))
...
MaplibreMap(
    ...
    stationList.forEach { station ->
        val bikeStation = rememberGeoJsonSource(
            id = station.name,
            data = GeoJsonData.Features(Point(station.position()))
        )
